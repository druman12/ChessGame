...
export default function AddFriendPopup(props) {
    const { userdetails } = useAuth();
    const [searchTerm, setSearchTerm] = useState('');
    const [loading, setLoading] = useState(false);

    const filteredEmails = props.emails.filter(email => 
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-center p-4" onClick={props.onClose}>
            <div className="w-full max-w-md bg-slate-800 rounded-2xl border border-slate-700 shadow-2xl" onClick={(e) => e.stopPropagation()}>
                <div className="p-6">
                    ...
                    <div className="mt-6">
                        <Button variant="secondary" onClick={props.onClose} className="w-full">
                            Close
                        </Button>
                    </div>
                </div>
